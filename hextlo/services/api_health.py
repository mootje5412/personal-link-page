...
async def _ping(endpoint: str, query: str) -> tuple[bool, str, int]:
    ...
    if isinstance(results, dict) and results.get("error") and results.get("error") is not False:
        msg = str(results.get("message") or results.get("error"))
        if msg == "INTELIUS_BLOCKED":
            msg = "blocked (anti-bot)"
        return False, msg, 0
...
async def check_all_apis() -> list[tuple[str, bool, str, int]]:
    checks = [
        ...
        ("NPD Search", SearchType.PERSON.value, "John,Doe,CA,90210"),
