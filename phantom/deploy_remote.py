#!/usr/bin/env python3
"""Deploy Phantom PWA to 109.71.252.128 — replaces existing server files."""
...
if [ ! -f /etc/ssl/certs/phantom.crt ]; then
  openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
    -keyout /etc/ssl/private/phantom.key \
    -out /etc/ssl/certs/phantom.crt \
    -subj "/CN=109.71.252.128/O=Phantom" \
    -addext "subjectAltName=IP:109.71.252.128"
  chmod 600 /etc/ssl/private/phantom.key
fi
"""
...
def main() -> int:
    if not LOCAL_DIST.is_dir():
        ...
        print("Run: cd phantom && npm install && npm run build", file=sys.stderr)
        return 1
    ...
    run(client, f"cat > /etc/nginx/sites-available/phantom << 'EOF'\n{NGINX_SITE}EOF")
    run(client, "rm -f /etc/nginx/sites-enabled/default /etc/nginx/sites-enabled/*")
    run(client, "ln -sf /etc/nginx/sites-available/phantom /etc/nginx/sites-enabled/phantom")
    run(client, "nginx -t")
    ...
    client.close()
    print(f"\nDone. Phantom PWA is live at https://{SERVER}/")
    return 0
